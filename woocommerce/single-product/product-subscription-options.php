...
<div class="wcsatt-options-wrapper <?php echo esc_attr( implode( ' ', $wrapper_classes ) ); ?>" data-sign_up_text="<?php echo esc_attr( $sign_up_text ); ?>" <?php echo $hide_wrapper ? 'style="display:none;"' : ''; ?>>
	...
	<div class="wcsatt-options-product-wrapper" <?php echo in_array( 'closed', $wrapper_classes ) ? 'style="display:none;"' : '' ?>><?php
		...
		?><ul class="wcsatt-options-product wcsatt-options-product--<?php echo $display_dropdown ? 'hidden' : ''; ?>"><?php
			foreach ( $options as $option ) {
				// two_write_to_file( $option['description'] );
				$description  = str_replace( ' &mdash;', '<span class="subscription-off">', $option[ 'description' ] );
				$description .= '</span>';
				?><li class="<?php echo esc_attr( $option[ 'class' ] ); ?>">
					<label>
						...
						<span class="<?php echo esc_attr( $option[ 'class' ] ) . '-details'; ?>"><?php echo $description; ?></span>
						<?php echo ''; ?>
					</label>
				</li><?php
			}
